...
@section('content')
    <div class="container mx-auto p-6">
        <h1 class="text-2xl font-bold mb-4">{{ $attempt->quiz->title }}</h1>
        <p class="mb-2">
            Status:
            <strong class="{{ $attempt->status === 'passed' ? 'text-green-600' : 'text-red-600' }}">
                {{ ucfirst($attempt->status) }}
            </strong>
        </p>
        <p class="mb-2">Score: <strong>{{ $attempt->score }} / {{ $attempt->total_points }} ({{ $attempt->percentage }}%)</strong></p>
