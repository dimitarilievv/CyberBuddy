    ...
    <div class="max-w-xl mx-auto py-6">
        ...
        <form method="POST" action="{{ route('modules.store') }}" class="space-y-4">
            ...
            <div>
                ...
                <input class="w-full border px-3 py-2 rounded" name="title" id="title" required value="{{ old('title') }}">
            </div>

            <div>
                <label class="block text-sm font-bold mb-1" for="description">Description</label>
                <textarea class="w-full border px-3 py-2 rounded" name="description" id="description" rows="3">{{ old('description') }}</textarea>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input class="w-full border px-3 py-2 rounded" name="difficulty" id="difficulty" value="{{ old('difficulty') }}">
                </div>
                <div>
                    <label class="block text-sm font-bold mb-1" for="age_group">Age Group</label>
                    <input class="w-full border px-3 py-2 rounded" name="age_group" id="age_group" value="{{ old('age_group') }}">
                </div>
            </div>

            <div>
                <label class="block text-sm font-bold mb-1" for="estimated_duration">Estimated Duration</label>
                <input class="w-full border px-3 py-2 rounded" name="estimated_duration" id="estimated_duration" value="{{ old('estimated_duration') }}">
            </div>

            <button class="bg-blue-600 text-white font-bold px-5 py-2 rounded" type="submit">Create Module</button>
